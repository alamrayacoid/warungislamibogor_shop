@if($group != '[]')
<div class="box-item-product-group-allstatus" id="testingbismillah">
    @foreach($group as $row)
    @if($row->sell_ccustomer == Auth::user()->cm_code)
    <div class="column-group-item-product item-product-allstatus mt-5">
        <div class="row">
            ...
            <div class="col-lg-4 col-md-4">
                <a data-target="#modal-detail" data-id="{{$row->sell_nota}}" data-status="{{$row->sell_status}}"
                    ...
                    data-metode="{{$row->sell_method}}" data-hargat="Rp. {{$row->sell_total}}" data-toggle="modal"
                    class="detail"><button class="btn btn-view-more-all-transaction">Lihat Detail
                        Transaksi</button></a>
            </div>
        </div>
        @foreach($allstatus as $roww)
        @if($roww->sell_nota == $row->sell_nota)
        <div class="row column-item-product itemproduct-group-allstatus">
            <div class="col-lg-6">
                <div class="d-flex">
